...
public class UserInterfaceObjectEditor {
	...
	void initNewWindow() {
		...
		if (realData == null)
			throw new RuntimeException("RealEstateObject is NULL");
		
		final JFrame jFrame = new JFrame("Редагування");
		...
		if (errorDescription != null) {
			labelErrorDescription = new JLabel("Помилки");
		...
		// constraintsLabel.anchor = GridBagConstraints.EAST;
	...
	private void readDataToRealEstateObject() {
		...
		String realEstateTypeID = UserInterfaceConstants.getRealEstateTypeIDByName(
				(String) comboREAType.getSelectedItem());
		realData.setRealEstateType(realEstateTypeID);
		
		realData.setRooms(fieldRooms.getText());
		
		String roomsTypeID = UserInterfaceConstants.getRoomsTypeIDByName(
				(String) comboRoomsType.getSelectedItem());
