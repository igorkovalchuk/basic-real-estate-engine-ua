 ...
 */
public class DBFStructureRowReaderPD001 extends DBFStructureRowReader001 {
	...
	static {
		...
		columns1.add(FLOOR);
		columns1.add(FLOORS);
		columns1.add(SQ_ALL_2);
		columns1.add(SQ_LIVE_2);
		columns1.add(SQ_LAND);
		columns1.add(INFORMATION_2);
	...
	protected RealEstateObject readAnObject(byte[] bytes, int rowOffset, DBFStructureAdapter adapter) throws ProgramException {
		...
		if (object != null) {
			...
			if ("дом".equals(type)) {
				object.setRealEstateType(INPUT_REAL_ESTATE_OBJECT_TYPE_HOUSE);
			} else if ("зем".equals(type)) {
				object.setRealEstateType(INPUT_REAL_ESTATE_OBJECT_TYPE_LAND);
			} else {
				Log.warn("Unknown real estate object type, (file 'PD'): " + type);
				object.setRealEstateType(INPUT_REAL_ESTATE_OBJECT_TYPE_HOUSE);
			}
			...
			if (
				...
				Log.warn("Unsupported combination, (file 'PD'): rent land");
			}
		}
